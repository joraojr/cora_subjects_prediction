...
if __name__ == "__main__":
    ...
    # Create DiGraph (paper2 ->paper1) using networkx
    G = nx.from_pandas_edgelist(cora_cites, source="citing", target="cited", create_using=nx.DiGraph)

    # Coloring the graph to extract the groups
    graph_color = pd.DataFrame.from_dict(nx.coloring.greedy_color(G), orient='index')
    ...
    # Transform group color into categorical data
    graph_color = pd.get_dummies(graph_color[0], prefix='group')
    tf = pd.concat([tf, graph_color], axis=1)
