...
if __name__ == "__main__":
    ...
    # Counting citing/cited class (i.e. mapping the node degrees by class) in the citation graph.
    ...
    # Row rescale to ensure be between 0 and 1
    citing_class[:] = MinMaxScaler().fit_transform(citing_class.T).T
    cited_class[:] = MinMaxScaler().fit_transform(cited_class.T).T

    concat_citations = pd.concat([citing_class, cited_class], axis=1)
